...
export class ValidationEngine {
    ...
    public async validateDocument(document: vscode.TextDocument): Promise<void> {
        try {
            ...
            console.log(`[ValidationEngine] File extension: ${extension}`);
            const extensions = ['itm', 'haz', 'rsk', 'sgl', 'req', 'blk', 'fma', 'fmc', 'tst', 'vcf', 'fml', 'fun', 'fta'];
            
            if (extensions.includes(extension || '')) {
                console.log(`[ValidationEngine] Using extension-based validation for .${extension} file`);
                // Use appropriate validator based on extension
                let validator: SafetyValidator | HazardValidator | RiskValidator | SafetyGoalsValidator | RequirementsValidator | BlockValidator | FailureModeAnalysisValidator | FailureModeControlsValidator | TestValidator | VariantConfigValidator | FunctionsValidator | FeaturesValidator;
                if (extension === 'haz') {
                    ...
                    const funConfig = getLanguageConfig('sylang-functions');
                    validator = new FunctionsValidator(funConfig!, this.symbolManager);
                    console.log(`[ValidationEngine] Running FunctionsValidator for .${extension} file`);
                } else if (extension === 'fml') {
                    const fmlConfig = getLanguageConfig('sylang-features');
                    validator = new FeaturesValidator(fmlConfig!, this.symbolManager);
                    console.log(`[ValidationEngine] Running FeaturesValidator for .${extension} file`);
                } else {
                ...
                // NEW: Add config-aware validation for .blk files (additive)
                if (extension === 'blk') {
                    console.log(`[ValidationEngine] Running config-aware validation for .${extension} file`);
                    const configAwareValidator = new BlockValidator(this.symbolManager);
                    const configAwareDiagnostics = await configAwareValidator.validateWithConfigAwareness(document);
                    diagnostics = diagnostics.concat(configAwareDiagnostics);
                    console.log(`[ValidationEngine] Added ${configAwareDiagnostics.length} config-aware diagnostics`);
                }
            } else {
                ...
                diagnostics = await validator.validate(document);
                console.log(`[ValidationEngine] Standard validation complete, found ${diagnostics.length} issues`);
                
                // NEW: Add config-aware validation for .blk files (additive)
                console.log(`[ValidationEngine] Checking if config-aware validation needed for extension: ${extension}`);
                if (extension === 'blk') {
                    console.log(`[ValidationEngine] Running config-aware validation for .${extension} file`);
                    // Create a dedicated BlockValidator instance for config-aware validation
                    const configAwareValidator = new BlockValidator(this.symbolManager);
                    const configAwareDiagnostics = await configAwareValidator.validateWithConfigAwareness(document);
                    diagnostics = diagnostics.concat(configAwareDiagnostics);
                    console.log(`[ValidationEngine] Added ${configAwareDiagnostics.length} config-aware diagnostics`);
                } else {
                    console.log(`[ValidationEngine] Skipping config-aware validation for extension: ${extension}`);
                }
            }
    ...
    public async validateWorkspace(): Promise<void> {
        try {
            const sylangFiles = await vscode.workspace.findFiles('**/*.{itm,haz,rsk,sgl,req,blk,fma,fmc,tst,vcf,ple,fml,vml,fun,fta}', '**/node_modules/**');
