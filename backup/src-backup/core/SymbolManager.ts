...
export class SymbolManager {
    ...
    // NEW: Config-aware functionality (additive, doesn't change existing behavior)
    ...
    constructor() {
        // NEW: Initialize config state manager (additive)
        this.configStateManager = new ConfigStateManager();
        
        // NEW: Listen for config changes to update symbol availability (additive)
        this.configStateManager.onConfigChanged(() => {
            this.refreshSymbolConfigStates();
        });
    }
    ...
    private parseBlockDocument(document: vscode.TextDocument): { definitions: SymbolDefinition[], references: SymbolReference[] } {
        ...
        for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
            ...
            // Parse port definition: def port out <identifier>
            else if (line.startsWith('def port out')) {
                const portMatch = line.match(/^def\s+port\s+out\s+([A-Za-z_][A-Za-z0-9_]*)$/);
                if (portMatch) {
            ...
            // Parse port in references: port in PowerSupply_12V, SystemClock, UserCommands
            else if (line.startsWith('port in ')) {
                const portInMatch = line.match(/port\s+in\s+(.+)/);
                if (portInMatch) {
                    const portList = portInMatch[1].split(',').map(s => s.trim());
                    for (const portName of portList) {
                        if (portName && /^[A-Za-z_][A-Za-z0-9_]*$/.test(portName)) {
                            const startPos = line.indexOf(portName);
                            references.push({
                                name: portName,
                                location: new vscode.Location(document.uri, new vscode.Position(lineIndex, startPos)),
                                context: 'port_in',
                                range: new vscode.Range(lineIndex, startPos, lineIndex, startPos + portName.length)
                            });
                        }
                    }
                }
    ...
    public async buildWorkspaceIndex(): Promise<void> {
        ...
        try {
            ...
            await vscode.window.withProgress({
                ...
                // NEW: Refresh config states when workspace changes (additive)
                await this.configStateManager.refreshConfigStates();
    ...
    /**
     * Find a symbol in workspace by file extension
     * Used for .blk files to search across all workspace .blk files
     */
    public findSymbolInWorkspace(symbolName: string, fileExtension: string): SymbolDefinition | null {
        // Search through all workspace symbols for files with the specified extension
        for (const [fileUri, symbols] of this.workspaceSymbols.entries()) {
            if (fileUri.endsWith(fileExtension)) {
                const foundSymbol = symbols.find(sym => sym.name === symbolName);
                if (foundSymbol) {
                    return foundSymbol;
                }
            }
    ...
    /**
     * Get header definition by identifier
     * Used for checking unused imports by examining child symbols
     */
    public getHeaderDefinition(identifier: string): HeaderDefinition | null {
        return this.workspaceIndex.headerDefinitions.get(identifier) || null;
    }

    // ============================================================================
    // NEW: Config-aware methods (additive functionality, doesn't change existing)
    // ============================================================================

    /**
     * NEW: Get available symbols filtered by config state (grayed out symbols excluded)
     * This is the config-aware version of getAvailableSymbols()
     */
    public getActiveSymbols(documentUri: string): SymbolDefinition[] {
        const allSymbols = this.getAvailableSymbols(documentUri); // Reuse existing method
        return allSymbols.filter(symbol => !this.isSymbolGrayedOutByConfig(symbol));
    }

    /**
     * NEW: Find symbol in workspace excluding grayed out symbols
     * This is the config-aware version of findSymbolInWorkspace()
     */
    public findActiveSymbolInWorkspace(symbolName: string, fileExtension: string): SymbolDefinition | null {
        const symbol = this.findSymbolInWorkspace(symbolName, fileExtension); // Reuse existing method
        if (symbol && this.isSymbolGrayedOutByConfig(symbol)) {
            return null; // Symbol exists but is grayed out
        }
        return symbol;
    }

    /**
     * NEW: Check if a symbol is grayed out based on config state
     */
    private isSymbolGrayedOutByConfig(symbol: SymbolDefinition): boolean {
        const configName = this.extractConfigFromSymbol(symbol);
        const isGrayedOut = this.configStateManager.isSymbolGrayedOut(configName);
        console.log(`[SymbolManager] Symbol '${symbol.name}' config='${configName}' grayedOut=${isGrayedOut}`);
        return isGrayedOut;
    }

    /**
     * NEW: Extract config name from a symbol by analyzing its file content
     * This finds the 'config c_ConfigName' line within the symbol's def block
     */
    private extractConfigFromSymbol(symbol: SymbolDefinition): string | undefined {
        try {
            // First check if config is already stored in symbol properties
            const configProperty = symbol.properties.get('config');
            console.log(`[SymbolManager] Symbol '${symbol.name}' has config property: '${configProperty}'`);
            if (configProperty) {
                return configProperty;
            }

            // If not in properties, try to parse from file content synchronously
            const fileUri = symbol.location.uri.toString();
            console.log(`[SymbolManager] No config property for '${symbol.name}', checking file: ${fileUri}`);
            
            // For now, return undefined - we'll enhance this later if needed
            return undefined;
        } catch (error) {
            console.error('[Sylang] Error extracting config from symbol:', error);
            return undefined;
        }
    }

    /**
     * NEW: Parse config from symbol's file location
     * This analyzes the file content around the symbol to find config usage
     */
    private async parseConfigFromSymbolLocation(symbol: SymbolDefinition): Promise<string | undefined> {
        try {
            const document = await vscode.workspace.openTextDocument(symbol.location.uri);
            const text = document.getText();
            const lines = text.split('\n');
            
            // Find the def line for this symbol
            const startLine = symbol.location.range.start.line;
            let endLine = lines.length;
            
            // Find the end of this def block (next def or end of file)
            for (let i = startLine + 1; i < lines.length; i++) {
                const line = lines[i].trim();
                if (line.startsWith('def ') && this.calculateIndentLevel(lines[i]) <= this.calculateIndentLevel(lines[startLine])) {
                    endLine = i;
                    break;
                }
            }
            
            // Look for config line within this def block
            ...
            console.error('[Sylang] Error parsing config from symbol location:', error);
            return undefined;
        }
    }

    /**
     * NEW: Helper method to calculate indentation level
     */
    ...
    /**
     * NEW: Refresh symbol config states when .vcf files change
     */
    private async refreshSymbolConfigStates(): Promise<void> {
        // Trigger re-parsing of documents to update symbol-config associations
        // This ensures symbol availability is updated when configs change
        console.log('[Sylang] Config changed, refreshing symbol states...');
        
        // Update config information for all symbols
        for (const [fileUri, symbols] of this.workspaceSymbols.entries()) {
            if (fileUri.endsWith('.blk')) {
                for (const symbol of symbols) {
                    // Re-extract config information for each symbol
                    const configName = await this.parseConfigFromSymbolLocation(symbol);
                    if (configName) {
                        symbol.properties.set('config', configName);
                    }
                }
            }
        }
        
        // Trigger re-validation for all open .blk files
        for (const editor of vscode.window.visibleTextEditors) {
            if (editor.document.fileName.endsWith('.blk')) {
                // Trigger document validation refresh
    ...
    /**
     * NEW: Get config state manager (for use by other components)
     */
    public getConfigStateManager(): ConfigStateManager {
