...
export class VariantModelValidator extends BaseValidator {
    ...
    protected async validateLanguageSpecificRules(
        ...
        // Validate standard properties (name, description, owner, tags, etc.)
            ...
            // Use base class property validation which has been fixed
            ...
            if (trimmedLine && !trimmedLine.startsWith('//') && !trimmedLine.startsWith('use ')) {
                if (!trimmedLine.startsWith('def variantmodel')) {
                    this.addDiagnostic(
                        ...
                        0,
                        trimmedLine.length,
                        'Variant model file must start with "def variantmodel <identifier>" (imports with "use" keyword are allowed before)',
    ...
    private async validateIndentationConsistency(document: vscode.TextDocument): Promise<void> {
        ...
        for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
            ...
            // No additional indentation validation needed - BaseValidator handles this
        }
    }
